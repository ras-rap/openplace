// pages/canvas/[id].tsx (complete updated version)
...
import { Home, Info, Palette, ZoomIn, ZoomOut, RotateCcw, Menu, X, Lock, Clock, Wifi, WifiOff, Users } from "lucide-react";
...
export default function Canvas() {
  ...
  const { isConnected, lastMessage, userCount } = useWebSocket(
    typeof id === 'string' ? id : '',
    'anonymous' // TODO: Replace with actual user ID
  );
  ...
  const loadCanvas = async (canvasId: string) => {
    try {
      ...
      setViewState({
        x: -canvas.width / 2,
        y: -canvas.height / 2,
        scale: Math.min(window.innerWidth / canvas.width, window.innerHeight / canvas.height) * 0.8,
      });
  ...
  const drawCanvas = useCallback(() => {
    ...
    if (hoveredPixel && currentTool === 'draw') {
      ...
      ctx.strokeRect(x - 0.5 / viewState.scale, y - 0.5 / viewState.scale, 
                     1 + 1 / viewState.scale, 1 + 1 / viewState.scale);
      
      // Add a white outline for better visibility
      ctx.strokeStyle = "#FFFFFF";
      ctx.lineWidth = Math.max(0.05, 1 / viewState.scale);
      ctx.strokeRect(x - 1 / viewState.scale, y - 1 / viewState.scale, 
                     1 + 2 / viewState.scale, 1 + 2 / viewState.scale);
    }

    // Restore context
    ctx.restore();
  }, [viewState, pixels, hoveredPixel, selectedColor, currentTool, canvasConfig, cooldownRemaining]);
  ...
  const screenToCanvas = useCallback(
    (screenX: number, screenY: number) => {
      ...
      if (canvasX >= 0 && canvasX < canvasConfig.width && canvasY >= 0 && canvasY < canvasConfig.height) {
  ...
  const placePixel = useCallback(
    async (x: number, y: number) => {
      ...
      if (timeSinceLastPlace < canvasConfig.placeCooldown) {
        setCooldownRemaining(Math.ceil(canvasConfig.placeCooldown - timeSinceLastPlace));
        return;
      }
    ...
    [selectedColor, canvasConfig, cooldownRemaining, lastPlaceTime, id]
  );
  ...
  const handleMouseMove = (event: React.MouseEvent) => {
    ...
    if (isPanning && currentTool === 'pan') {
      ...
      setLastPanPoint({ x: event.clientX, y: event.clientY });
    } else if (isPlacing && currentTool === 'draw') {
      if (coords) {
        placePixel(coords.x, coords.y);
      }
    }
  ...
  const handleWheel = useCallback((event: WheelEvent) => {
    ...
    const newScale = Math.max(canvasConfig.minZoom, Math.min(canvasConfig.maxZoom, viewState.scale * delta));
  ...
  const resetView = () => {
    ...
    setViewState({
      ...
      scale: Math.min(window.innerWidth / canvasConfig.width, window.innerHeight / canvasConfig.height) * 0.8,
    });
  };
  ...
  }, [showNavbar, showColorPicker, showZoomControls]);
  ...
  return (
    <>
      <Head>
        <title>{canvasConfig?.name} - OpenPlace</title>
        <meta name="description" content={`Collaborative pixel art canvas: ${canvasConfig?.name}`} />
      </Head>
      ...
      >
        ...
        <Button
          variant="outline"
          size="sm"
          className="fixed top-4 left-4 z-50 bg-white/90 backdrop-blur-sm border-2 border-gray-300 shadow-lg hover:bg-white hover:shadow-xl transition-all duration-200 dark:bg-gray-900/90 dark:border-gray-600 dark:hover:bg-gray-900"
          onClick={() => setShowNavbar(!showNavbar)}
        >
          {showNavbar ? <X className="h-4 w-4" /> : <Menu className="h-4 w-4" />}
        </Button>
        ...
        >
          <div className="border-b bg-background/95 py-4 backdrop-blur-sm shadow-lg">
            <div className="container mx-auto flex items-center justify-between px-4">
              ...
              <nav className="flex items-center space-x-4">
                ...
                <Button variant="ghost" size="sm" asChild>
                  <Link href="/canvases">Explore</Link>
                </Button>
                <Button variant="ghost" size="sm" asChild>
        ...
        >
          <div className="bg-white/95 backdrop-blur-sm rounded-full px-6 py-4 shadow-2xl border-2 border-gray-200 dark:bg-gray-900/95 dark:border-gray-700">
                ...
                <>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="h-8 w-8 cursor-pointer rounded-full border-2 border-gray-300 bg-transparent"
                    />
        ...
        <Button
          variant="outline"
          size="sm"
          className={`fixed bottom-4 left-4 z-50 bg-white/90 backdrop-blur-sm border-2 border-gray-300 shadow-lg hover:bg-white hover:shadow-xl transition-all duration-200 dark:bg-gray-900/90 dark:border-gray-600 dark:hover:bg-gray-900 ${
            showColorPicker ? "opacity-50" : ""
          }`}
          onClick={() => setShowColorPicker(!showColorPicker)}
        >
          <div 
            className="h-4 w-4 rounded-full border border-gray-400 mr-2"
        ...
        <Button
          variant="outline"
          size="sm"
          className="fixed bottom-4 right-4 z-50 bg-white/90 backdrop-blur-sm border-2 border-gray-300 shadow-lg hover:bg-white hover:shadow-xl transition-all duration-200 dark:bg-gray-900/90 dark:border-gray-600 dark:hover:bg-gray-900"
        ...
        {/* Zoom Controls */}
        <div
          className={`fixed bottom-20 right-4 z-40 transform transition-all duration-200 ${
            showZoomControls ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"
          }`}
        >
        ...
        <div className="fixed top-4 right-4 z-40 flex flex-col space-y-2">
          <div className="rounded-lg bg-black/80 backdrop-blur-sm p-2 text-sm text-white shadow-xl border border-gray-600 flex items-center space-x-2">
            {isConnected ? (
          ...
          <div className="rounded-lg bg-black/80 backdrop-blur-sm p-2 text-sm text-white shadow-xl border border-gray-600">
          ...
          {cooldownRemaining > 0 && (
            <div className="rounded-lg bg-red-600/90 backdrop-blur-sm p-2 text-sm text-white shadow-xl border border-red-500 flex items-center space-x-2">
        ...
        <div className="fixed top-20 left-4 z-40 rounded-lg bg-black/80 backdrop-blur-sm p-3 text-sm text-white shadow-xl border border-gray-600">
          <div><strong>{canvasConfig?.name}</strong></div>
          <div>Size: {canvasConfig?.width}×{canvasConfig?.height}</div>
          <div>Cooldown: {canvasConfig?.placeCooldown}s</div>
          {canvasConfig?.allowedColors && (
            <div>Colors: Restricted ({canvasConfig.allowedColors.length})</div>
          )}
          <div className="mt-2"><strong>Controls:</strong></div>
          <div>Left click: Place pixel</div>
